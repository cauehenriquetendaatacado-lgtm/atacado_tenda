...
- **Senha comum** (funcionários/gerentes de loja): só vê e usa o formulário
  de registro de ocorrência — não enxerga o resumo, os gráficos, os painéis
  por regional/loja nem a lista de registros de outras pessoas (o servidor
  também bloqueia a leitura dos registros para essa senha).
- **Senha de gerência**: enxerga o resumo e todos os gráficos do dashboard
  (ocorrências registradas, valor recuperado total e do mês, gráfico por
  regional, estatística mensal de ocorrências inibidas por regional, apoio
  de cópia/quadrilha/monitoramento) — a mesma parte que o administrador vê
  no topo do painel — e também o painel de regionais, com os totais de cada
  uma das 4 regionais e de cada loja (ocorrências, inibidas, não inibidas,
  quadrilha, valor recuperado). Não vê o formulário de registro, a lista de
  ocorrências recentes, nem o detalhe de cada ocorrência individual (loja a
  loja, com descrição, foto e quem registrou) — essa última tela fica
  restrita ao administrador. O servidor só entrega, para essa senha, um
  resumo sem os campos sensíveis (nome de quem registrou, descrição, foto e
...
- **Senha de administrador**: enxerga tudo isso, vê quem registrou cada
  ...
  registro para auditar o que foi lançado, e pode navegar pelos painéis de
  regional/loja e pela lista completa de ocorrências.
...
- Senha de gerência: `gerencia123@` (variável de ambiente `SENHA_GERENCIA`)
...
administrador. Quem entrar com a senha de gerência vê um selo "Gerência"
e só o resumo/gráficos do dashboard. Quem entrar com a senha comum vê
apenas o formulário de registro.
...
4. Em "Environment", adicione as variáveis `SENHA_PAINEL` (senha comum),
   `SENHA_GERENCIA` (senha de gerência) e `SENHA_ADMIN` (senha de
