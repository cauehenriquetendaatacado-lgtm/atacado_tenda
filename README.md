...
- **Senha comum** (funcionários): só vê e usa o formulário de registro de
  ocorrência, com o campo de anexar foto — não enxerga a lista de
  registros de outras pessoas, os painéis por regional/loja nem os
  gráficos.
- **Senha de gerente**: vê exatamente o mesmo formulário de registro da
  senha comum, mas **sem o campo de anexar foto**.
...
- Senha de gerente: `gerente123@` (variável de ambiente `SENHA_GERENTE`)
...
administrador. Quem entrar com a senha comum vê apenas o formulário de
registro, com o campo de foto. Quem entrar com a senha de gerente vê um
selo "Gerente" e o mesmo formulário, só que sem o campo de foto.
...
4. Em "Environment", adicione as variáveis `SENHA_PAINEL` (senha comum),
   `SENHA_GERENTE` (senha de gerente) e `SENHA_ADMIN` (senha de
