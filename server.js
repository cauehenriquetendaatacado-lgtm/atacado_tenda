...
// ---------- Autenticacao ----------
// Tres senhas compartilhadas: a normal (funcionarios), a da gerencia (so
// consulta os paineis, nao registra) e a de administrador. Todas dao acesso
// ao painel; so a de administrador libera as acoes de auditoria (confirmar
// registros, excluir).
function checarSenha(req, res, next) {
  const senha = req.headers["x-senha"];
  if (senha !== SENHA && senha !== SENHA_ADMIN && senha !== SENHA_GERENCIA) {
    return res.status(401).json({ erro: "Senha incorreta" });
  }
  req.ehAdmin = senha === SENHA_ADMIN;
  req.ehGerencia = senha === SENHA_GERENCIA;
  next();
}
...
app.post("/api/ocorrencias", checarSenha, async (req, res) => {
  // A gerencia so consulta os paineis; nao registra ocorrencias.
  if (req.ehGerencia) {
    return res.status(403).json({ erro: "A gerencia nao registra ocorrencias" });
  }
